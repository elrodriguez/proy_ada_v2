				...
				for(var i = 0 ; i<valores.length; i++){
					...
					if(valores[i]['num_proceso'] =='4'){
						if (valores[i][5]=="RECHAZADO") {
							...
							cadena += `<button onclick='rechazarproceso("`+valores[i][0]+`","ACEPTADO")'>Aceptar</button>
							<button onclick='rechazarproceso("`+valores[i][0]+`","RECHAZADO")'>Rechazar</button>`;
						}else{
...
function rechazarproceso(codigo){
	swal({
		title: "¿Estas segur@?",
		text: "Una vez rechazado, ¡no podrá restaurar!",
		icon: "warning",
		buttons: true,
		dangerMode: true,
	}).then((willDelete) => {
