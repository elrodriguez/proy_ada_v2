	...
	$.ajax({
		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				cadena += "<th style = 'text-align: center;color: #fff;width: 20px;word-wrap: break-word;'>CARGAR RESOLUCIÓN</th>";
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					cadena += `<td style = 'text-align: center;width: 150px;word-wrap: break-word;'>
						<input class="input-calendar" type="date" id="date-`+valores[i][0]+`" onchange="registrarfechas(event,'1','`+valores[i][0]+`')" value="`+valores[i]['fecha_aprobada']+`">
					</td>`;
					cadena += `<td style = 'text-align: center;width: 150px;word-wrap: break-word;'>
					...
					if(valores[i]['num_proceso'] =='5'){
							cadena += `<button class="btn btn-aceptar btn-sx" onclick='rechazarproceso("`+valores[i][0]+`","PENDIENTE","6")'>Aceptar</button>`;
					}else{
...
function AbrirModalSubirArchivoAnexos(control){
	...
	$('#modal-subir-anexos-resolucion').modal({backdrop: 'static', keyboard: false})
	$('#modal-subir-anexos-resolucion').modal('show');
}
function registrar_documento_anexos_resolucion_subir(){
	console.log('dddd');
	$.ajax({
	  type : 'POST',
	  url:'../controlador/documento/controlador_registrar_documento_coordinador_resolucion.php',
	  data:  new FormData(document.getElementById("form-upload-file-anexos-resolucion")),
	  contentType: false,
	  cache: false,
	  processData:false,
	  success:function(resp) {
		if(resp>0){
		  $('#modal-subir-anexos-resolucion').modal('hide');
			document.getElementById("form-upload-file-anexos-resolucion").reset();
		  	swal("Anexos Registrado!", "", "success").then ( ( value ) =>  {
				listar_documento_vista_revisor('','1')
		  	});
		}else{
		  var filet1 = $("#file-a1").val();
		  if (filet1 == null || filet1 == '') {
