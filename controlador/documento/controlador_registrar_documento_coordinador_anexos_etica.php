<?php
	...
	if ($_FILES["file-a4v2"]['tmp_name']!="") {
    	$file_type = $_FILES['file-a4v2']['type'];
        list($type, $extension) = explode('/', $file_type);
		if($extension == 'pdf'){
			$imagen3=uniqid()."-".$_FILES['file-a4v2']['name'];
			$ruta3=$_FILES['file-a4v2']['tmp_name'];
	...
	if ($_FILES["file-a4v3"]['tmp_name']!="") {
    	$file_type = $_FILES['file-a4v3']['type'];
        list($type, $extension) = explode('/', $file_type);
		if($extension == 'pdf'){
			$imagen4=uniqid()."-".$_FILES['file-a4v3']['name'];
			$ruta4=$_FILES['file-a4v3']['tmp_name'];
	...
	$consulta = $MC->subir_documento_anexos(4,$iddocumento,$nombre1,$nombre2,$nombre3,$nombre4);
	echo $consulta;
?>
