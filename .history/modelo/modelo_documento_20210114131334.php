<?php
	class Modelo_documento
	{
		...
		function registrar_jurado($iddocumento,$revisor,$tipo){
			$fecha = date('Y-m-d');
			$sql = "INSERT INTO documento_jurado (documento_cod,asesor_cod,fecha_registro,tipo) VALUES ('$iddocumento','$revisor','$fecha','$tipo')";
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
			else{
