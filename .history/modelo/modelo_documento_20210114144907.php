<?php
	class Modelo_documento
	{
		...
		function registrar_revisor($iddocumento,$revisor,$tipo,$modalidad){
			$fecha = date('Y-m-d');
			$sql = "INSERT INTO documento_revisor (documento_cod,asesor_cod,fecha_registro,tipo) VALUES ('$iddocumento','$revisor','$fecha','$tipo','$modalidad')";
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
			else{
		...
		function registrar_jurado($iddocumento,$revisor,$tipo){
			$fecha = date('Y-m-d');
			$sql = "INSERT INTO documento_jurado (documento_cod,asesor_cod,fecha_registro,tipo) VALUES ('$iddocumento','$revisor','$fecha','$tipo','$modalidad')";
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
			else{
