	...
	class Modelo_documento
	{
		...
		function listar_documento($valor, $inicio=FALSE,$limite=FALSE){
		 ...
		 function listar_documento_revisor($valor, $inicio=FALSE,$limite=FALSE){
			if ($inicio!==FALSE && $limite!==FALSE) {
			    $sql = "SELECT documento.documento_cod, documento.doc_asunto,documento.doc_fecha_recepcion,tipo_documento.tipodo_descripcion,area.area_nombre,documento.doc_estado,documento.doc_tipo,area.area_cod,tipo_documento.tipodocumento_cod,IFNULL(documento.doc_documento,''),CONCAT(t5.nombre,' ',t5.apellido_pater,' ',t5.apellido_mater) AS asesor_full_name,porcentaje,archivo_turniting,paso,fecha_revisor_correo FROM documento INNER JOIN tipo_documento ON documento.tipoDocumento_cod = tipo_documento.tipodocumento_cod INNER JOIN area ON documento.area_cod = area.area_cod INNER JOIN asesor AS t5 ON documento.asesor_cod=t5.asesor_cod WHERE documento.paso='3' AND documento.documento_cod LIKE '".$valor."%' ORDER BY documento.documento_cod DESC LIMIT $inicio,$limite";
			}else{
		...
		function cambiar_paso_documento($iddocumento){
			$sql = "UPDATE documento SET paso=3,doc_estado = 'PENDIENTE' WHERE documento_cod = '$iddocumento'";
			//echo $sql;exit;
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
