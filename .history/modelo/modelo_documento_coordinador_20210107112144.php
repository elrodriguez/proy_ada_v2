	...
	class Modelo_documento
	{
		...
		function listar_documento($valor, $inicio=FALSE,$limite=FALSE){
		 ...
		 function listar_documento_revisor($valor, $inicio=FALSE,$limite=FALSE){
			if ($inicio!==FALSE && $limite!==FALSE) {
			    $sql = "SELECT documento.documento_cod, documento.doc_asunto,documento.doc_fecha_recepcion,tipo_documento.tipodo_descripcion,area.area_nombre,documento.doc_estado,documento.doc_tipo,area.area_cod,tipo_documento.tipodocumento_cod,IFNULL(documento.doc_documento,''),CONCAT(t5.nombre,' ',t5.apellido_pater,' ',t5.apellido_mater) AS asesor_full_name,porcentaje,archivo_turniting,num_proceso,fecha_revisor_correo,fecha_final,TIMESTAMPDIFF(DAY, NOW(), fecha_final) AS dias_restantes,anexo_uno,anexo_seis,estado_paso_tres,num_proceso,anexo_seis_2,anexo_seis_3 FROM documento INNER JOIN tipo_documento ON documento.tipoDocumento_cod = tipo_documento.tipodocumento_cod INNER JOIN area ON documento.area_cod = area.area_cod INNER JOIN asesor AS t5 ON documento.asesor_cod=t5.asesor_cod WHERE documento.num_proceso='3' AND documento.documento_cod LIKE '".$valor."%' ORDER BY documento.documento_cod DESC LIMIT $inicio,$limite";
			}else{
		...
		function listar_documento_revisor_generar_resolucion($valor, $inicio=FALSE,$limite=FALSE){
			if ($inicio!==FALSE && $limite!==FALSE) {
			    $sql = "SELECT documento.documento_cod, documento.doc_asunto,documento.doc_fecha_recepcion,tipo_documento.tipodo_descripcion,area.area_nombre,documento.doc_estado,documento.doc_tipo,area.area_cod,tipo_documento.tipodocumento_cod,IFNULL(documento.doc_documento,''),CONCAT(t5.nombre,' ',t5.apellido_pater,' ',t5.apellido_mater) AS asesor_full_name,porcentaje,archivo_turniting,num_proceso,fecha_revisor_correo,fecha_final,TIMESTAMPDIFF(DAY, NOW(), fecha_final) AS dias_restantes,anexo_uno,anexo_seis,etica_anexo_uno,etica_anexo_cuatro,estado_paso_tres,num_proceso,resolucion_firmada FROM documento INNER JOIN tipo_documento ON documento.tipoDocumento_cod = tipo_documento.tipodocumento_cod INNER JOIN area ON documento.area_cod = area.area_cod INNER JOIN asesor AS t5 ON documento.asesor_cod=t5.asesor_cod WHERE documento.num_proceso='5' AND documento.documento_cod LIKE '".$valor."%' ORDER BY documento.documento_cod DESC LIMIT $inicio,$limite";
			}else{
		 ...
		 function listar_documento_etica($valor, $inicio=FALSE,$limite=FALSE){
			if ($inicio!==FALSE && $limite!==FALSE) {
			    $sql = "SELECT documento.documento_cod, documento.doc_asunto,documento.doc_fecha_recepcion,tipo_documento.tipodo_descripcion,area.area_nombre,documento.doc_estado,documento.doc_tipo,area.area_cod,tipo_documento.tipodocumento_cod,IFNULL(documento.doc_documento,''),CONCAT(t5.nombre,' ',t5.apellido_pater,' ',t5.apellido_mater) AS asesor_full_name,porcentaje,archivo_turniting,num_proceso,fecha_revisor_correo,fecha_final,TIMESTAMPDIFF(DAY, NOW(), fecha_final) AS dias_restantes,anexo_uno,anexo_seis,etica_anexo_uno,etica_anexo_cuatro,estado_paso_cuatro,num_proceso,etica_anexo_cuatro_dos,etica_anexo_cuatro_tres FROM documento INNER JOIN tipo_documento ON documento.tipoDocumento_cod = tipo_documento.tipodocumento_cod INNER JOIN area ON documento.area_cod = area.area_cod INNER JOIN asesor AS t5 ON documento.asesor_cod=t5.asesor_cod WHERE documento.num_proceso='4' AND documento.documento_cod LIKE '".$valor."%' ORDER BY documento.documento_cod DESC LIMIT $inicio,$limite";
			}else{
		...
		function Rechazado_documento($iddocumento,$estado=''){
		...
		function subir_documento_anexos($flag,$iddocumento,$destino1,$destino2,$destino3 = ''){
			$sql = "call PA_SUBIRARCHIVOANEXOS('$flag','$iddocumento','$destino1','$destino2','$destino3')";
			//print_r($sql);exit;
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
		...
		function aprobar_pagar_revisor_documento($iddocumento,$estado){
			$sql = "UPDATE documento_revisor SET por_pagar = '$estado' WHERE documento_cod = '$iddocumento'";
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
			else{
