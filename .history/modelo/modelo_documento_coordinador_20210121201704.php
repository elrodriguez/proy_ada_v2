	...
	class Modelo_documento
	{
		...
		function listar_documento($valor, $inicio=FALSE,$limite=FALSE){
		 ...
		 function listar_documento_revisor($valor, $inicio=FALSE,$limite=FALSE){
			if ($inicio!==FALSE && $limite!==FALSE) {
			    $sql = "SELECT documento.documento_cod, documento.doc_asunto,documento.doc_fecha_recepcion,tipo_documento.tipodo_descripcion,area.area_nombre,documento.doc_estado,documento.doc_tipo,area.area_cod,tipo_documento.tipodocumento_cod,documento.doc_documento,porcentaje,archivo_turniting,num_proceso,fecha_revisor_correo,fecha_final,TIMESTAMPDIFF(DAY, NOW(), fecha_final) AS dias_restantes,anexo_uno,anexo_seis,estado_paso_tres,num_proceso,anexo_seis_2,anexo_seis_3,archivo_etapa1_v2,archivo_etapa1_v3 FROM documento INNER JOIN tipo_documento ON documento.tipoDocumento_cod = tipo_documento.tipodocumento_cod WHERE documento.num_proceso='2' AND documento.documento_cod LIKE '".$valor."%' ORDER BY documento.documento_cod DESC LIMIT $inicio,$limite";
			}else{
			    $sql = "SELECT documento.documento_cod, documento.doc_asunto,documento.doc_fecha_recepcion,tipo_documento.tipodo_descripcion,area.area_nombre,documento.doc_estado,documento.doc_tipo,area.area_cod,tipo_documento.tipodocumento_cod,documento.doc_documento,porcentaje,archivo_turniting,num_proceso,fecha_revisor_correo,fecha_final,TIMESTAMPDIFF(DAY, NOW(), fecha_final) AS dias_restantes,anexo_uno,anexo_seis,estado_paso_tres,num_proceso,anexo_seis_2,anexo_seis_3,archivo_etapa1_v2,archivo_etapa1_v3 FROM documento INNER JOIN tipo_documento ON documento.tipoDocumento_cod = tipo_documento.tipodocumento_cod WHERE documento.num_proceso='2' AND documento.documento_cod LIKE '".$valor."%' ORDER BY documento.documento_cod DESC";
			}
			$resultado =  $this->conexion->conexion->query($sql);
			$arreglo = array();
			while($consulta_VU=mysqli_fetch_array($resultado)){ ///MYSQL_BOTH, MYSQL_ASSOC, MYSQL_NUM
