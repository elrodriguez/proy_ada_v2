<?php
	class Modelo_documento
	{
		...
		function traer_revisor($codigo){
			$sql = "SELECT asesor_cod,nombre,apellido_pater,apellido_mater,dni,(SELECT COUNT(asesor_cod) FROM documento_revisor AS t2 WHERE t2.asesor_cod=t1.asesor_cod AND t2.documento_cod='".$codigo."') AS revisor FROM asesor AS t1 WHERE tipo_id = 1;";
			//echo $sql;exit;
			$arreglo = array();
			if ($consulta = $this->conexion->conexion->query($sql)) {
