<?php
	class Modelo_documento
	{
		...
		function traer_jurado($codigo){
			$sql = "SELECT asesor_cod,nombre,apellido_pater,apellido_mater,dni,celular,(SELECT COUNT(asesor_cod) FROM documento_revisor AS t2 WHERE t2.asesor_cod=t1.asesor_cod AND t2.documento_cod='".$codigo."') AS revisor FROM asesor AS t1 INNER JOIN asesor_tipo_detalle AS t4 ON t1.asesor_cod = t4.id_asesor WHERE t4.id_tipo = 3;";
			//echo $sql;exit;
			$arreglo = array();
			if ($consulta = $this->conexion->conexion->query($sql)) {
		...
		function registrar_revisor($iddocumento,$revisor,$tipo){
			$fecha = date('Y-m-d');
			$sql = "INSERT INTO documento_revisor (documento_cod,asesor_cod,fecha_registro,tipo) VALUES ('$iddocumento','$revisor','$fecha','$tipo')";
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
			else{
