<?php
	class Modelo_documento
	{
		...
		function listar_jurado($codigo){
			$sql = "SELECT t1.id,t2.nombre,t2.apellido_pater,t2.apellido_mater,t2.dni,t2.celular,t1.tipo,t1.modalidad FROM documento_jurado AS t1 INNER JOIN asesor AS t2 ON t1.asesor_cod=t2.asesor_cod WHERE documento_cod='".$codigo."';";
			//echo $sql;exit;
			$arreglo = array();
			if ($consulta = $this->conexion->conexion->query($sql)) {
		...
		function listar_asesor($codigo){
			$sql = "SELECT t1.id,t2.nombre,t2.apellido_pater,t2.apellido_mater,t2.dni,t2.celular,t1.tipo,t1.modalidad FROM documento_asesor AS t1 INNER JOIN asesor AS t2 ON t1.asesor_cod=t2.asesor_cod WHERE documento_cod='".$codigo."';";
			//echo $sql;exit;
			$arreglo = array();
			if ($consulta = $this->conexion->conexion->query($sql)) {
		...
		function listar_revisor($codigo){
			$sql = "SELECT t1.id,t2.nombre,t2.apellido_pater,t2.apellido_mater,t2.dni,t2.celular,t1.tipo,t1.modalidad FROM documento_revisor AS t1 INNER JOIN asesor AS t2 ON t1.asesor_cod=t2.asesor_cod WHERE documento_cod='".$codigo."';";
			//echo $sql;exit;
			$arreglo = array();
			if ($consulta = $this->conexion->conexion->query($sql)) {
		...
		function registrar_jurado($iddocumento,$revisor,$tipo,$modalidad){
			$sql = "CALL PA_REGISTRARJURADODOCUMENTO('$iddocumento','$revisor','$tipo','$modalidad')";
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
			else{
