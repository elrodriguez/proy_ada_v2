<?php
	class Modelo_personal
	{
		...
		function editar_docente($id,$nombre,$apePat,$apeMat,$telefo,$movil,$direcc,$fecnac,$dni,$email,$genero,$estado,$tipos,$txtcategoria){
			$sql = "call PA_EDITARDOCENTE('$id','$nombre','$apePat','$apeMat','$telefo','$movil','$direcc','$fecnac','$dni','$email','$genero','$estado','$tipos','$txtcategoria')";
			//echo $sql;exit;
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
		...
		function listar_combo_tipo_docente(){
		...
		function registrar_docente($nombre,$apePat,$apeMat,$telefo,$movil,$direcc,$fecnac,$dni,$email,$genero,$estado,$tipos,$categoria){
			$sql = "CALL PA_REGISTRARDOCENTE('$nombre','$apePat','$apeMat','$telefo','$movil','$direcc','$fecnac','$dni','$email','$genero','$estado','$tipos','$categoria')";
			echo $sql;exit;
			if ($resultado = $this->conexion->conexion->query($sql)){
				return 1;
			}
		...
		function listar_docente($valor, $inicio=FALSE,$limite=FALSE){
			if ($inicio!==FALSE && $limite!==FALSE) {
			    $sql = "SELECT * FROM asesor where asesor.dni like '".$valor."%' or asesor.nombre like '".$valor."%' ORDER BY asesor.asesor_cod DESC LIMIT $inicio,$limite";
			}else{
			    $sql = "SELECT * FROM asesor where asesor.dni like '".$valor."%' or asesor.nombre like '".$valor."%' ORDER BY asesor.asesor_cod DESC";
			}
			$resultado =  $this->conexion->conexion->query($sql);
			$arreglo = array();
			while($consulta_VU=mysqli_fetch_array($resultado)){ ///MYSQL_BOTH, MYSQL_ASSOC, MYSQL_NUM
		 ...
		 function listar_combo_tipo_docente_detalle($id){
			$sql = "SELECT * from asesor_tipo_detalle where id_asesor = '$id'";
