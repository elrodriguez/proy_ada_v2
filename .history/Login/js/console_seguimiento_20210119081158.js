			...
			for (var i = 0; i < valores.length; i++) {
				cadena += "<tr>";			
					...
					if(valores[i]['num_proceso'] == '7' && valores[i]['doc_estado'] == 'RECHAZADO'){
						...
						btn_subir_1_etapa7 = `<button type="button" class="btn btn-primary" name="`+valores[i][0]+`*`+valores[i]['anexo_dies_dos']+`" onclick="modalsubircorrecionesetapacuatro(this,'13')">
												<i class="glyphicon glyphicon-open"></i>
											  </button>`;
						btn_subir_2_etapa7 = `<button type="button" class="btn btn-primary" name="`+valores[i][0]+`*`+valores[i]['anexo_dies_tres']+`" onclick="modalsubircorrecionesetapacuatro(this,'14')">
												<i class="glyphicon glyphicon-open"></i>
					  						</button>`;
					}else{
						btn_ver_1_etapa7 = '';
