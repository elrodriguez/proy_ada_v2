			...
			for (var i = 0; i < valores.length; i++) {
				cadena += "<tr>";			
					...
					if(valores[i]['num_proceso'] == '4' && valores[i]['doc_estado'] == 'RECHAZADO'){
						btn_subir_1_etapa4 = `<button type="button" class="btn btn-primary" name="`+valores[i][0]+`*`+valores[i]['anexo_uno_etapa_cuatro']+`" onclick="modalsubircorrecionesetapacuatro(this,'A1')">
												<i class="glyphicon glyphicon-open"></i>
											  </button>`;
						btn_subir_2_etapa4 = `<button type="button" class="btn btn-primary" name="`+valores[i][0]+`*`+valores[i]['carta_etapa_cuatro']+`" onclick="modalsubircorrecionesetapacuatro(this,'C')">
												<i class="glyphicon glyphicon-open"></i>
					  						</button>`;
					}else{
						btn_subir_1_etapa4 = '';
...
function modalsubircorrecionesetapacuatro(control,tipo){
	...
	if(tipo == '9'){
		...
		$('#file-ax1-e4-title').val('9')
	}else{
		$('#file-ax1-e4-title').html('Carta')
		$('#file-ax1-e4-title').val('10')
	}
