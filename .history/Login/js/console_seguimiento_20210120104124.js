			...
			for (var i = 0; i < valores.length; i++) {
				cadena += "<tr>";			
					...
					if(valores[i]['num_proceso'] == '9' && valores[i]['doc_estado'] == 'RECHAZADO'){
						btn_ver_1_etapa9 = `<a target="_blank" href="../`+valores[i]['anexo_dies']+`" class='btn btn-primary btn-sx' style='background-color:#fff;border-color:#fff' title='Ver documento Cargado'><i class='fa  fa-folder-open' style='color:orange;'></i></a>`;
						btn_subir_1_etapa9 = `<button type="button" class="btn btn-primary" name="`+valores[i][0]+`*`+valores[i]['anexo_dies_tres']+`" onclick="modalsubircorrecionesetapacuatro(this,'15')">
												<i class="glyphicon glyphicon-open"></i>
					  						</button>`;
					}else{
						btn_ver_1_etapa9 = '';
