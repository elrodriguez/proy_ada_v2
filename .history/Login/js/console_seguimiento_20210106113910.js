			...
			for (var i = 0; i < valores.length; i++) {
				cadena += "<tr>";			
					...
					if(valores[i]['num_proceso'] == '3' && valores[i]['doc_estado'] == 'RECHAZADO'){
						btn_subir_nuevos = `<button type="button" class="btn btn-primary" onclick="modalsubircorreciones('`+valores[i][0]+`')">
												Subir Correcciones
					  						</button>`;
					}
...
function modalsubircorreciones(documento){

	$('#iddocumentovcorrecciones').val(documento);
