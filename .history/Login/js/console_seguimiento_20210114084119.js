			...
			for (var i = 0; i < valores.length; i++) {
				cadena += "<tr>";			
					...
					if(valores[i]['num_proceso'] == '4' && valores[i]['doc_estado'] == 'RECHAZADO'){
						btn_subir_1_etapa4 = `<button type="button" class="btn btn-primary" name="`+valores[i][0]+`*`+valores[i]['anexo_uno_etapa_cuatro']+`" onclick="modalsubircorrecionesetapacuatro(this)">
												<i class="glyphicon glyphicon-open"></i>
											  </button>`;
						btn_subir_2_etapa4 = `<button type="button" class="btn btn-primary" name="`+valores[i][0]+`*`+valores[i]['carta_etapa_cuatro']+`" onclick="modalsubircorrecionesetapacuatro(this)">
												<i class="glyphicon glyphicon-open"></i>
					  						</button>`;
					}else{
						btn_subir_1_etapa4 = '';
...
function modalsubircorrecionesetapacuatro(control){
	var datos = control.name;
	var datos_split = datos.split("*");
	if(datos_split[1]!='0'){
		var btnverv1 = `<a target="_blank" href="../`+datos_split[1]+`" class="btn btn-default" type="button">Ver</a>`;
