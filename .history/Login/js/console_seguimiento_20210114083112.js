			...
			for (var i = 0; i < valores.length; i++) {
				cadena += "<tr>";			
					...
					if(valores[i]['num_proceso'] == '4' && valores[i]['doc_estado'] == 'RECHAZADO'){
						btn_subir_nuevos_etapa4 = `<button type="button" class="btn btn-primary" name="`+valores[i][0]+`*`+valores[i]['anexo_uno_etapa_tres']+`*`+valores[i]['proyecto_etapa_tres']+`*`+valores[i]['carta_etapa_tres']+`" onclick="modalsubircorreciones(this)">
												<i class="glyphicon glyphicon-open"></i>
					  						</button>`;
					}else{
						btn_subir_nuevos_etapa4 = '';
					}
					cadena += "<td style = 'text-align: center;width: 20px;word-wrap: break-word;'>"+btn_subir_nuevos_etapa4+"</td>";
