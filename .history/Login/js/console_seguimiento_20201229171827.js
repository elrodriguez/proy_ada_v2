		...
		if (valores.length > 0) {
			...
			for (var i = 0; i < valores.length; i++) {
				cadena += "<tr>";			
					...
					if(valores[i]['doc_estado']=="RECHAZADO" && valores[i]['num_proceso']==1 && valores[i]['anexo_uno']== null){
						btn_v2 = `<button name='`+valores[i][0]+`*`+valores[i][1]+`*`+valores[i][2]+`*`+valores[i][3]+`*`+valores[i][4]+`*`+valores[i][5]+`*`+valores[i][6]+`*`+valores[i][7]+`*`+valores[i][8]+`*`+valores[i][9]+`*`+valores[i][10]+`*`+valores[i][11]+`*`+valores[i][12]+`*`+valores[i][13]+`*`+valores[i][14]+`' class='btn btn-primary' onclick='AbrirModalSubirArchivoAnexos(this)'><i class='glyphicon glyphicon-upload'></i></button>`;
					}else{
						btn_v2 = '<i class="glyphicon glyphicon-remove"></i>';
					}
					if(valores[i]['doc_estado']=="RECHAZADO" && valores[i]['num_proceso']==1){
...
function registrar_documento_anexos(){
	$.ajax({
	  type : 'POST',
	  url:'controlador/seguimiento/controlador_registrar_documento_coordinador_anexos.php',
