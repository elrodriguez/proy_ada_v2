function listar_documento_vista_revisor(valor,pagina){
	...
	$.ajax({
		url:'../controlador/documento/controlador_ListarBuscar_documento_coordinador_revisor_generar_resolucion.php',
		type: 'POST',
		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				cadena += "<th style = 'text-align: center;color: #fff;width: 20px;word-wrap: break-word;'>Anexo A1 - A4</th>";
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					cadena += `<td style = 'text-align: center;width: 150px;word-wrap: break-word;'>
						<input type="date" id="date-`+valores[i][0]+`" onchange="registrarfechas(event,'2','`+valores[i][0]+`')" value="`+valores[i]['fecha_entrega']+`">
					</td>`;
					...
					if(valores[i]['num_proceso'] =='5'){
						...
						//}else if (valores[i][5]=="PENDIENTE") {
							cadena += `<button onclick='rechazarproceso("`+valores[i][0]+`","PENDIENTE","6")'>Aceptar</button>
							<button onclick='rechazarproceso("`+valores[i][0]+`","RECHAZADO","5")'>Rechazar</button>`;
						//}else{
						//	cadena += `<button onclick='rechazarproceso("`+valores[i][0]+`","RECHAZADO","6")'>Rechazar</button>`;
						//}
					}else{
...
function generar_resolucion_word(documento){
	$('#iframe-word-descarga').attr('src','../controlador/documento/controlador_generar_resolucion_revisor.php?doc='+documento);
}
