		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					cadena += `<td style = 'text-align: center;width: 20px;word-wrap: break-word;'><button type='button' class='btn btn-link' onclick='modalverturniting("`+valores[i]['porcentaje']+`","`+valores[i][0]+`","`+valores[i]['archivo_turniting']+`")' >Ver</button></td>`;
