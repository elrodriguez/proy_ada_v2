...
function enviarcorreopordocumento(control){
	var datos = control.name;
	$.get( "../controlador/documento/controlador_enviar_correo.php?doc="+datos, function( data ) {
		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					cadena += "<td><button name='"+valores[i][0]+"' class='btn btn-primary' onclick='enviarcorreopordocumento(this)'><i class='fa fa-envelope'></i>";
...
function BuscarRemitenteCiudadano(control) {
	...
	.done(function(resp) {
		...
		if (data.length > 0) {
			...
			var cadena= $('#tbody-tabla-alumnos-tesis').html();
