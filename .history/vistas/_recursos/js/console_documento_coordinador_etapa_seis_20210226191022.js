		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					cadena += `<td><button class='btn btn-primary btn-sx' style='background-color:#fff;border-color:#fff' type="button" onclick="abrirmodalcombodetalle('`+valores[i][0]+`')" name="`+valores[i][0]+`*`+valores[i]['tipo_publicacion']+`*`+valores[i]['nombre_revista']+`"><i class="glyphicon glyphicon-edit" style='color:#000000;'></i></button></td>`;
...
function abrirmodalcombodetalle(id){
	var datos = control.name;
	var datos_split = datos.split("*");
	$('#tipo_publicacion').val(datos_split[1]);
	$('#nombre_revista').val(datos_split[2]);
	$('#tipopublicacioniddociuemneto').val(id);
	$('#myModalLabelComboDrop').html(id);
