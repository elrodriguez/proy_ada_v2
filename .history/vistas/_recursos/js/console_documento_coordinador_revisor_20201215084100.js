			...
			if(valores.length>0){
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					if(valores[i]['anexo_uno']){
						btn_ver += `<button type='button' class='btn btn-link' onclick='modalverturniting("A1","`+valores[i][0]+`","`+valores[i]['anexo_uno']+`")' >A1</button>`;
					}
					if(valores[i]['anexo_seis']){
						btn_ver += `&nbsp;-&nbsp;<button type='button' class='btn btn-link' onclick='modalverturniting("A6","`+valores[i][0]+`","`+valores[i]['anexo_seis']+`")' >A6</button>`;
					}
...
function modalveranexos(porcentaje,title,url){
	var contenidotitulo = '<p><strong>'+title+'</strong> Porcentaje: '+porcentaje+'</p>';
	var iframehtml = `<iframe id="iframemodalturniting" src="../controlador/documento/`+url+`" width="100%" height="500" frameborder="0" scrolling="no"></iframe>`;
	$('#modal-title-tirniting').html(contenidotitulo);
	$('#modal-body-turniting').html(iframehtml);
