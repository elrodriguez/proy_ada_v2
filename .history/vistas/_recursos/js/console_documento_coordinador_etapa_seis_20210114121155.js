	...
	$.ajax({
		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				cadena += "<th style = 'text-align: center;color:#fff;width: 150px;word-wrap: break-word;'>JURADO</th>";
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					if(valores[i]['anexo_siete'] != '0' && valores[i]['anexo_ocho'] != '0'){
						cadena += `<button name='agregar_revisor' class='btn btn-primary btn-sx' style='background-color:#fff;border-color:#fff' title='ASIGNAR REVISOR' onclick='AbrirModalRevisorAgregar("`+valores[i][0]+`")'><i class='glyphicon glyphicon-user' style='color:#000000;'></i></button>`;
					}
...
function AbrirModalRevisorAgregar(control){
	...
	$.ajax({
		url:'../controlador/documento/controlador_documento_traeremitente_revisor.php',
		type:'POST',
		data:{
			codigo:control
		}
