...
function Listar_programa_academico() {
	$.ajax({
		...
		if (data.length > 0) {
			var cadena = "";
				cadena += "<option value='otro'>SELECCIONAR</option>";
			var grado = '';
			for (var i = 0; i < data.length; i++) {
				if(grado != data[i][2]){
					cadena +=" <optgroup label='"+data[i][2]+"'>";
					for (var c = 0; c < data.length; i++) {
						if(data[c][2]==data[i][2]){
							cadena += "<option value='"+data[c][0]+"'>"+data[c][1]+"</option>";
						}
					}
					cadena +=" </optgroup>";
				}
