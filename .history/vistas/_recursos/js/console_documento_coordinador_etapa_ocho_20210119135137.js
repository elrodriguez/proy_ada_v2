		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					cadena += `<td><input onchange="marcarAnexoSubido('`+valores[i][0]+`','14',this)" type="checkbox" `+check1+`></td>`;
					cadena += `<td><input onchange="marcarAnexoSubido('`+valores[i][0]+`','15',this)" type="checkbox" `+check2+`></td>`;
...
function marcarAnexoSubido(codigo,flag,check){
	if(check.checked){
        ...
        alert("El checkbox con valor " + check.checked + " ha sido seleccionado");
    } else {
        // Hacer algo si el checkbox ha sido deseleccionado
        alert("El checkbox con valor " + check.checked + " ha sido deseleccionado");
	}
