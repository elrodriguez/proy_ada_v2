	...
	$.ajax({
		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				cadena += "<th style = 'text-align: center;color: #fff;width: 20px;word-wrap: break-word;'>CARGAR anexo A6</th>";
				cadena += "<th colspan='3' style = 'text-align: center;color: #fff;width: 20px;word-wrap: break-word;'>Correciones</th>";
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					if(valores[i]['anexo_uno_etapa_tres']){
						btn_ver_uno += `<button type='button' class='btn btn-link' onclick='modalveranexos("A1","`+valores[i][0]+`","`+valores[i]['anexo_uno']+`")' >A1</button>`;
					}
					if(valores[i]['proyecto_etapa_tres']){
						btn_ver_pro += `<button type='button' class='btn btn-link' onclick='modalveranexos("Proyecto","`+valores[i][0]+`","`+valores[i]['anexo_uno']+`")' >Proyecto</button>`;
					}
					if(valores[i]['carta_etapa_tres']){
						btn_ver_car += `<button type='button' class='btn btn-link' onclick='modalveranexos("Carta","`+valores[i][0]+`","`+valores[i]['anexo_uno']+`")' >Carta</button>`;
					}
