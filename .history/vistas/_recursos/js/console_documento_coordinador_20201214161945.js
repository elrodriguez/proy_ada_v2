		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					cadena +="</td>";
					cadena += "<td style = 'text-align: center;width: 10px;word-wrap: break-word;'><button name='"+valores[i][0]+"*"+valores[i][1]+"*"+valores[i][2]+"*"+valores[i][3]+"' class='btn btn-primary' onclick='AbrirModalDocumento(this)'><span class='glyphicon glyphicon-envelope'></span>";
					cadena += "</button></td> ";
					cadena += "</tr>";
				}
				cadena += "</tbody>";
		...
		if (data.length > 0) {
			...
			for (var i = 0; i < data.length; i++) {
				cadena+=`<tr>
							...
							<td class="text-center" onclick="eliminarrevisordocumento('`+data[i]['id']+`')"><i class="glyphicon glyphicon-remove"></i></td>
						</tr>`;
			}
