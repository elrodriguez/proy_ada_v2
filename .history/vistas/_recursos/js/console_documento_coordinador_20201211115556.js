	...
	$.ajax({
		...
		if (data.length > 0) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				cadena+=`<option `+data[i]['revisor']+` value="`+data[i]['asesor_cod']+`">`+data[i]['dni']+` - `+data[i]['nombre']+` `+data[i]['apellido_pater']+` `+data[i]['apellido_mater']+`</option>`;
			}
			$('#select-revisor').html(cadena)
		}else{
