		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					cadena += `<td><button class='btn btn-primary btn-sx' style='background-color:#fff;border-color:#fff' type="button" onclick="abrirmodalcombodetalle('`+valores[i][0]+`')"><i class="glyphicon glyphicon-edit" style='color:#000000;'></i></button></td>`;
...
function abririnputadicional(){
	...
	alert(tt)
	if( tt == '2' && tt == '3'){
		$('#div_nombre_revista').attr("style", "display:block");
	}else{
		$('#div_nombre_revista').attr("style", "display:none");
	}
}