		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					cadena += `<td style = 'text-align: center;width: 20px;word-wrap: break-word;'><button name='agregar_revisor' class='btn btn-primary btn-sx' style='background-color:#fff;border-color:#fff' title='ASIGNAR REVISOR' onclick='AbrirModalRevisorAgregar("`+valores[i][9]+`")'><i class='glyphicon glyphicon-user' style='color:#000000;'></i></button></td>`;
		...
		if (data.length > 0) {
			...
			for (var i = 0; i < data.length; i++) {
				cadena+=`<tr>
							...
							if(data[i]['revisor'] == '0'){
								cadena+=`<span style="cursor:pointer" title="Agregar" class="glyphicon glyphicon-ok"></span>`;
							}else{
								cadena+=`<span style="cursor:pointer" title="Quitar" class="glyphicon glyphicon-remove"></span>`;
							}
							cadena+=`</td>
						</tr>`;
			}
