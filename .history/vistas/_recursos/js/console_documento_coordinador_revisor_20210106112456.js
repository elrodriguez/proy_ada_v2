		...
		success: function(resp){
			...
			if(valores.length>0){
				...
				for(var i = 0 ; i<valores.length; i++){
					...
					if(valores[i]['estado_paso_tres']=="APROBADO"){
						btn_saltar  = `<button class='btn btn-success' onclick='pagarrevisor("`+valores[i][0]+`")'><i class='glyphicon glyphicon-thumbs-up'></i></button>`;
					}else{
						btn_saltar = '';
					}
					...
					if(valores[i]['num_proceso'] =='3'){
						if (valores[i][5]=="RECHAZADO") {
							...
							cadena += `<button onclick='rechazarproceso("`+valores[i][0]+`","ACEPTADO")'><i  class='glyphicon glyphicon-question-sign' style='color:#000000;'></i>Aceptar</button>
							<button onclick='rechazarproceso("`+valores[i][0]+`","RECHAZADO")'><i  class='glyphicon glyphicon-ok' style='color:#000000;'></i>Rechazar</button>`;
						}else{
							cadena += `<button onclick='rechazarproceso("`+valores[i][0]+`","RECHAZADO")'><i  class='glyphicon glyphicon-ok' style='color:#000000;'></i></button>`;
						}
					}else{
